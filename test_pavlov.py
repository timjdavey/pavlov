...
def test_normalised_dict():
    ...
    assert nd(['a','b','c','d']) == {'a':0.0, 'b':1/3, 'c':2/3, 'd':1.0}
...
def test_input_data_history():
    ...
    assert elephant.history == [low_action, high_action, low_action, low_action]
...
def test_input_data_failures():
    TEST_ACTIONS = [low_action, high_actmion]
    ENVIRON = {'a': 0.1, 'z': 0.9}
    ...
    with pytest.raises(KeyError):
        subject.input_data({'c': 1.0})

    # input value too high
    with pytest.raises(ValueError):
        subject.input_data({'a': 0.1, 'z': 1.2})
